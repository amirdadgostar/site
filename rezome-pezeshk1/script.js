 ...
 * VERSION: 3.7 (ULTIMATE PDF EDIT - HIGH QUALITY & 2-PAGE LAYOUT)
 * DESCRIPTION: Definitive Logic.
 *              - FAB Button logic preserved (10s delay).
 *              - vCard Toast message preserved.
 *              - PDF Generation logic REWRITTEN for High-Res, 2-Page Professional Layout.
 */
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // =======================================================
    // 9. تولید PDF حرفه‌ای (Resume Generation - UPDATED)
    // =======================================================
    window.generateFullPDF = function() {
        ...
        if (fabBtn) {
            ...
            fabBtn.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> در حال ایجاد...';
            fabBtn.disabled = true;
        }

        // استفاده از قالب جدید طراحی شده
        ...
        // نمایش موقت برای رندر صحیح
        pdfTemplate.style.display = 'block';

        // تنظیمات بهینه شده برای کیفیت بالا و سایز A4 دقیق
        const options = {
            margin: 0,
            filename: 'Dr-Sara-Radmanesh-Official-Resume.pdf',
            image: { type: 'jpeg', quality: 0.98 }, // کیفیت تصویر بالا
            html2canvas: {
                scale: 2.5, // اسکیل بالا برای جلوگیری از تاری متون
                useCORS: true, // برای لود تصاویر خارجی
                logging: false,
                letterRendering: true,
                scrollY: 0,
                windowWidth: 800 // عرض پنجره مجازی
            },
            jsPDF: {
                unit: 'mm',
                format: 'a4',
            ...
            // تنظیم شکست صفحات به صورت هوشمند
            pagebreak: { mode: ['avoid-all', 'css', 'legacy'] }
        };

        if (typeof html2pdf !== 'undefined') {
            html2pdf().set(options).from(pdfTemplate).save().then(() => {
                showToast('فایل رزومه با کیفیت بالا دانلود شد.', 'success');
                if (fabBtn) resetPdfButton(fabBtn, originalIcon);
